...
func TestAssertHasError(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithError(assert.AnError).Error()
	e := hook.LastEntry()
...
func TestAssertHasMonitoringEvent(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithMonitoringEvent("anEvent", "tid_test", "aContentType").Info()
	e := hook.LastEntry()
...
func TestAssertHasTime(t *testing.T) {
	...
	logger.WithTransactionID("tid_test").WithTime(expectedTime).Info()
	e := hook.LastEntry()
...
func TestAssertHasTimeFailed(t *testing.T) {
	...
	logger.WithTransactionID("tid_test").WithTime(time.Now()).Info()
	e := hook.LastEntry()
...
func TestAssertHasTransactionID(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithTransactionID("tid_test").Info()
	e := hook.LastEntry()
...
func TestAssertHasUUID(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithTransactionID("tid_test").WithUUID("dbc40c07-63ef-4ea3-82d6-a5a5d8747363").Info()
	e := hook.LastEntry()
...
func TestAssertHasValidFlagTrue(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithTransactionID("tid_test").WithValidFlag(true).Info()
	e := hook.LastEntry()
...
func TestAssertHasValidFlagFalse(t *testing.T) {
	mockT := new(testing.T)
	hook := NewTestHook("test_service")
	logger.WithTransactionID("tid_test").WithValidFlag(false).Info()
	e := hook.LastEntry()
	Assert(mockT, e).HasValidFlag(false)
	assert.False(t, mockT.Failed())
}
